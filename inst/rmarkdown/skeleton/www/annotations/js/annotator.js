...
var Adder = Widget.extend({
    ...
    // Public: Hide all elements related to imgselect when adder mode shows up.
    hide_imgselect: function() {
        
        $(".imgareaselect-outer").each(function() {
            $(this).css('display', 'none');
        });
        
        $('.annotator-viewer + div').css('display', 'none');
        $('.annotator-adder:eq(1)').addClass('annotator-hide');
    },
...
var Editor = exports.Editor = Widget.extend({
    ...
    execute_tmp_ann: function() {
        var self = this;
        
        $(document).on('mouseup', function(e){
            
            var editor_obj = $(e.target).closest(".annotator-editor");
            var comment_text = $('.annotator-editor li:first textarea').val();
            
            // Check if click was triggered on or within #annotator-editor
                if (editor_obj.length == 0 && comment_text.length == 0) {
                        self._onCancelNewTextClick(e);
                    }
                else if (editor_obj.length == 0 && comment_text.length > 0) {
                        self._onSaveNewTextClick(e);
                    }
        });
    },
    ...
    addField: function (options) {
        ...
        if (field.type === 'textarea') {
            ...
            input = $('<select />');
        } else if (field.type === 'textarea-recipient') {
            input = $('<textarea class="recipient"/>');
        }
    ...
    // Event callback: called when a user clicks the body area and at least one letter was added in comment.
    //
    // Returns nothing
    _onSaveNewTextClick: function (event) {
        preventEventDefault(event);
        this.submit();
    },

    // Event callback: called when a user clicks the body area and no letter was added in comment.
    //
    // Returns nothing
    _onCancelNewTextClick: function (event) {
        preventEventDefault(event);
        this.cancel();
    },
...
function main(options) {
        ...
        s.textselector = new textselector.TextSelector(options.element, {
            onSelection: function (ranges, event) {
                if (ranges.length > 0) {
                    ...
                    s.adder.hide_imgselect();

                } else {
                    console.log('adder-hide');
    ...
    return {
        ...
        annotationsLoaded: function (anns) { s.highlighter.drawAll(anns); console.log('annotationLoaded');},
        annotationCreated: function (ann) {/* s.highlighter.draw(ann); */ console.log('annotationCreated');},
        annotationDeleted: function (ann) { s.highlighter.undraw(ann); console.log('annotationDeleted');},
        annotationUpdated: function (ann) { s.highlighter.redraw(ann); console.log('annotationUpdated'); $('.annotator-viewer').css('display', 'block');},
        ...
        beforeAnnotationUpdated: function (annotation) {
            //return s.editor.load(annotation, s.interactionPoint);
 	        ...
 	        .catch( function (object) {
 	        	// remove highlighting for cancelled annotation
 	        	$('.annotator-viewer').css('display', 'block');
 	        	throw object;
 	        });
        }
    };
...
exports.editorExtension = function editorExtension(e) {
    ...
    field = e.addField({
        type: 'textarea-recipient',
        label: _t('Add email contacts') + '\u2026',
        load: updateField,
